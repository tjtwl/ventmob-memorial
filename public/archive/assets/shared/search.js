...
(function () {
  ...
  var PANEL_MAX_WIDTH = 560; // keep in sync with #vm-search-results max-width in search.css

  function positionPanel() {
    var r = input.getBoundingClientRect();
    panel.style.top = Math.round(r.bottom + 4) + 'px';
    panel.style.minWidth = Math.round(Math.max(r.width, 280)) + 'px';
    // Anchor from whichever side keeps the panel on-screen: some skins put the search box
    // hard against the right edge, where growing rightward from its left edge runs off-screen.
    if (window.innerWidth - r.left < PANEL_MAX_WIDTH) {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (path.indexOf(panel) === -1 && path.indexOf(input) === -1) {
      panel.hidden = true;
    }
  });
